...
框架默认不提供异常处理器，你可以定制自己的异常处理器。

## 自定义错误处理器

创建 `ErrorMiddleware.ts` 文件，定制自己的错误处理逻辑。
...
```ts filename = src/config/config.plugin.ts {6-9}
/**
 * yunfly 插件
 * 数组顺序就是插件的加载顺序
 */
...
3. `config/config.default.ts` 中启用插件 `config.error`
